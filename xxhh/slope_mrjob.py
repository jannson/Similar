...
class PredictBySlopeOne(MRJob):
    ...
    def reducer2(self, key, value):
        rateM = {}
        preM = []
        for item_id,rate in value:
            if rate == -1.0:
                preM.append(item_id)
            else:
                rateM[item_id] = rate
               
        # all item in the user
    ...
    # key is (itemi, itemj) value is all (user, diff, -1) 
    # calc the total diff and freq
    ...
    # The userid, item value: all rate,freq
